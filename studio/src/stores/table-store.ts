...
export interface TableStore {
  ...
  setRowSelection: (tableKey: string, rowSelection: Record<string, boolean>) => void
...
export const useTableStore = create<TableStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    setRowSelection: (tableKey: string, rowSelection: Record<string, boolean>) => {
        ...
        tabs: {
          ...
          selectedTables: prev.tabs.selectedTables.includes(tableKey)
            ...
            : prev.tabs.selectedTables.length >= actualMaxTabs
              ? [...prev.tabs.selectedTables.slice(0, actualMaxTabs - 1), tableKey]
    ...
    removeTable: (tableKey: string) => {
      set((prev) => {
        ...
        return {
          tabs: {
            ...prev.tabs,
            selectedTables: prev.tabs.selectedTables.filter((t) => t !== tableKey),
            activeTableKey: prev.tabs.activeTableKey === tableKey ? null : prev.tabs.activeTableKey,
          },
