...
import { ChevronRight, Search, SqlIcon, Plus, BookOpen } from '@hugeicons/core-free-icons'
      ...
      <div className="p-4 border-b flex flex-col gap-4">
        ...
        <div className="relative flex gap-2">
          <div className="relative flex-1">
            <HugeiconsIcon icon={Search} className="absolute left-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input 
              value={searchQuery}
          ...
          <Button variant="outline" size="icon" className="h-9 w-9 shrink-0" onClick={() => handleSelectQuery('', 'Untitled query')}>
              ...
              <SidebarGroupLabel render={
                <div className="flex w-full items-center justify-between pr-2">
                  <CollapsibleTrigger className="hover:text-accent-foreground text-muted-foreground cursor-pointer flex flex-1 items-center font-medium tracking-wide">
                    ...
                    />
                    PRIVATE {uniqueHistory.length > 0 && <span className="ml-1 opacity-70">({uniqueHistory.length})</span>}
                  </CollapsibleTrigger>
                  {uniqueHistory.length > 0 && (
                    <Button variant="ghost" size="sm" className="h-5 px-1.5 text-[10px]" onClick={onClear}>Clear</Button>
                  )}
                </div>
              } />
              <CollapsibleContent>
                  ...
                  <SidebarMenu className="gap-0.5">
                    {uniqueHistory.length === 0 ? (
                      <Empty className="py-6 border-0 bg-transparent rounded-none">
                        <EmptyHeader>
                          <EmptyTitle className="text-sm">No queries</EmptyTitle>
                          <EmptyDescription className="text-xs">
                            {searchQuery ? 'No queries match your search' : 'Your query history will appear here'}
                          </EmptyDescription>
                        </EmptyHeader>
                      </Empty>
                    ) : (
                      uniqueHistory.map((item) => (
                        <SidebarMenuItem key={item.id}>
                          <SidebarMenuButton
                            ...
                            onClick={() => handleSelectQuery(item.query, 'Untitled query')}
                            title={item.query}
                          >
                            <span className={cn('mr-2 shrink-0', item.success ? 'text-green-500/70' : 'text-destructive/70')}>
                              <HugeiconsIcon icon={SqlIcon} className="h-4 w-4" />
                            </span>
                    ...
                    {filteredTemplates.length === 0 ? (
                      <Empty className="py-6 border-0 bg-transparent rounded-none">
                        <EmptyHeader>
                          ...
                          <EmptyDescription className="text-xs">
                            {searchQuery ? 'No templates match your search' : 'No SQL templates available'}
                          </EmptyDescription>
                        </EmptyHeader>
                      </Empty>
                    ) : (
                      filteredTemplates.map((template) => (
                        <SidebarMenuItem key={template.id}>
                          <SidebarMenuButton
                            className="w-full justify-start py-1.5 hover:bg-accent/50 h-auto"
                            onClick={() => handleSelectQuery(template.query, template.name)}
                          >
